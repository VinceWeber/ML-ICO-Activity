USE [ICO_Activite]
GO
/****** Object:  StoredProcedure [dbo].[Preproc_A3_1_V6]    Script Date: 11/07/2023 16:42:28 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[Preproc_A3_1_V6] @TABLE_ACT nvarchar(50), @TABLE_UF nvarchar(50), @TABLE_SORTIE1 nvarchar(50),@TABLE_SORTIE2 as nvarchar(50)
...
SET @Query= @Query +' SELECT [' + @TABLE_ACT +' ].N_S,  max(' + @TABLE_UF +' .[Poids_acte]) as Max_Acte_Soins 
'
...
SET @Query= ' SELECT [' + @TABLE_ACT + '].N_S,  max(' + @TABLE_UF +'.[Poids_acte]) as Max_Acte_SOS 
'
...
SET @Query= ' SELECT min(cast([' + @TABLE_ACT +'].[ID_A] as float)) as Id_A_Caracteristique, [' + @TABLE_ACT +'].N_S 
'
SET @Query= @Query +' INTO ' + @TABLE_SORTIE1 + '
'
SET @Query= @Query +' FROM [' + @TABLE_ACT + '] INNER JOIN ' + @TABLE_INTERM +' ON [' + @TABLE_ACT +'].N_S = ' + @TABLE_INTERM + '.N_S INNER JOIN ' + @TABLE_UF + ' ON [' + @TABLE_ACT +'].UFX = ' + @TABLE_UF + '.UFX_Code 
'
SET @Query= @Query +' WHERE ' + @TABLE_UF + '.[Dimension_Parcours] like ' + @Dim_Parcours +' and ' + @TABLE_UF + '.[Poids_acte]= '+@TABLE_INTERM+'.Max_Acte_Soins
'
SET @Query= @Query +' GROUP BY [' + @TABLE_ACT +'].N_S
'
SET @Query= @Query +' ORDER BY [' + @TABLE_ACT +'].N_S
'
...
SET @Query= ' SELECT min(cast([' + @TABLE_ACT +'].[ID_A] as float)) as Id_Acte_Caracteristique, [' + @TABLE_ACT + '].N_S
'
SET @Query= @Query +' INTO ' + @TABLE_SORTIE2
SET @Query= @Query +' FROM [' + @TABLE_ACT + '] 
INNER JOIN ' + @TABLE_INTERM2 + ' ON [' + @TABLE_ACT +'].N_S = ' + @TABLE_INTERM2 +'.N_S INNER JOIN ' +
