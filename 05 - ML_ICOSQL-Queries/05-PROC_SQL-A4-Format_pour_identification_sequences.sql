USE [ICO_Activite]
GO
/****** Object:  StoredProcedure [dbo].[Preproc_A4_V6]    Script Date: 11/07/2023 16:44:17 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[Preproc_A4_V6] @TABLE_ACT nvarchar(50), @TABLE_UF nvarchar(50), @TABLE_SORTIE nvarchar(50), @TABLE_Poids_Sejours nvarchar(50),@TABLE_J0V1V2 nvarchar(50),
...
SET @Query= @Query + ' SELECT [' + @TABLE_ACT +'].NIP, [' + @TABLE_ACT +'].N_S, ' + @TABLE_Ddetfinsejour +'.Ddebsej, ' + @TABLE_J0V1V2 +'.J0_V1, ' + @TABLE_J0V1V2 + '.J0_V2, 
	SUBSTRING ([' + @TABLE_ACT +'].NIP,2,4) as Annee_NIP, ' + @TABLE_Poids_Sejours +'.Poids_Sejour_DS, ' + @TABLE_Poids_Sejours + '.Poids_Sejour_DSOS 
	'
SET @Query= @Query + 'INTO ' + @TABLE_SORTIE +'
'
SET @Query= @Query + 'FROM [' + @TABLE_ACT +'] INNER JOIN 
                         ' + @TABLE_Ddetfinsejour +' ON ['+ @TABLE_ACT +'].N_S = ' + @TABLE_Ddetfinsejour +'.N_S INNER JOIN
                         ' + @TABLE_J0V1V2 +' ON [' + @TABLE_ACT +'].NIP = '+@TABLE_J0V1V2 +'.NIP INNER JOIN
                         ' + @TABLE_Poids_Sejours +' ON [' + @TABLE_ACT +'].N_S = ' + @TABLE_Poids_Sejours +'.N_S
						 '
SET @Query= @Query + 'GROUP BY [' + @TABLE_ACT + '].NIP, [' + @TABLE_ACT +'].N_S, '+@TABLE_Ddetfinsejour +'.Ddebsej, ' + @TABLE_J0V1V2 +'.J0_V1, ' + @TABLE_J0V1V2 +'.J0_V2, 
                          ' + @TABLE_Poids_Sejours +'.Poids_Sejour_DS, ' + @TABLE_Poids_Sejours + '.Poids_Sejour_DSOS
						  '
SET @Query= @Query + 'ORDER BY NIP, N_S'
