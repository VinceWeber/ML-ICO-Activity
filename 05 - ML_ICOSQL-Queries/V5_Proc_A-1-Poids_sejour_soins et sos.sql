...
SET @Query = @Query + ' ' + 'SELECT ' + @TABLE_ACT + '.Ho_Num_Num_sejour, sum(0) as Poids_Sejour_DS, sum(' + @TABLE_UF +'.[Poids_acte]) as Poids_Sejour_DSOS, ' + @TABLE_UF +'.[Dimension_Parcours]
'
SET @Query = @Query + ' ' + 'FROM ' + @TABLE_ACT +' INNER JOIN ' + @TABLE_UF +' ON ' + @TABLE_ACT +'.UFX_UFX_Code = ' + @TABLE_UF +'.UFX_Code
'
SET @Query = @Query + ' ' + 'WHERE ' + @TABLE_UF + '.[Dimension_Parcours]=' + @Dim_Parcours
...
SET @Query = @Query + ' ' + 'SELECT ' + @TABLE_ACT + '.Ho_Num_Num_sejour, sum(0) as Poids_Sejour_DS, sum(0) as Poids_Sejour_DSOS, ' + @TABLE_UF +'.[Dimension_Parcours]
'
SET @Query = @Query + ' ' + 'FROM ' + @TABLE_ACT +' INNER JOIN ' + @TABLE_UF +' ON ' + @TABLE_ACT +'.UFX_UFX_Code = ' + @TABLE_UF +'.UFX_Code
'
