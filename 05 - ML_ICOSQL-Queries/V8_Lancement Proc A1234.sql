...
SET @TABLE_ACT='A_Actes_Export_09062020_SH'
...
--SET @Query ='SELECT TOP (500) * INTO dbo.A_Actes_Table_Analyse FROM ' + @TABLE_ACT + ' WHERE Source=''NGAP'''
--SET @Query += ' UNION '
--SET @Query +='SELECT TOP (500) * FROM ' + @TABLE_ACT + ' WHERE Source=''CCAM'''
--SET @Query += ' UNION '
--SET @Query +='SELECT TOP (500) * FROM ' + @TABLE_ACT + ' WHERE Source=''MVT'''
SET @Query = 'SELECT  * INTO dbo.A_Actes_Table_Analyse FROM ' + @TABLE_ACT
...
EXECUTE [dbo].[Preproc_B2_Category_V7] @TABLE_ACT , 'Tmp_UF_Table' , 'id_A','_UF' , 'UFX'
EXECUTE [dbo].[Preproc_B2_Category_V7] @TABLE_ACT , 'Tmp_INX_Table' , 'id_A','_INX' , 'INX'
EXECUTE [dbo].[Preproc_B2_Category_V7] @TABLE_ACT , 'Tmp_NGAP_Table' , 'id_A','_NGAP' , 'R_NGAP'
EXECUTE [dbo].[Preproc_B2_Category_V7] @TABLE_ACT , 'Tmp_CCAM_Table' , 'id_A','_CCAM' , 'R_CCAM'
EXECUTE [dbo].[Preproc_B2_Category_V7] @TABLE_ACT , 'Tmp_UFH_Table' , 'id_A','_UFH' , 'UFH'
EXECUTE [dbo].[Preproc_B2_Category_V7] @TABLE_ACT , 'Tmp_Code_Equip_Table' , 'id_A','_Eq' , 'Code_Equip'
EXECUTE [dbo].[Preproc_B2_Category_V7] @TABLE_SORTIE_B , 'Tmp_Type_Seq_Table' , 'id_Sequence' ,'', 'Type_Sequence'

EXECUTE [dbo].[Preproc_B2_Category_UF_With_Weight_V8] @TABLE_ACT, @TABLE_UF, 'Tmp_UF_Dim_Table' 
