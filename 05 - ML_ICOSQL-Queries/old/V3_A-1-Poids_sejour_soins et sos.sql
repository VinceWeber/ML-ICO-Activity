...
SET @Query = @Query + 'IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N' + @TABLE_SORTIE +') 
'
SET @Query = @Query + 'DROP TABLE [dbo].[' +@TABLE_SORTIE + ']'

PRINT '--DELETING PREVIOUS TABLES -----QUERY : 
' + @Query
EXEC (@Query);
