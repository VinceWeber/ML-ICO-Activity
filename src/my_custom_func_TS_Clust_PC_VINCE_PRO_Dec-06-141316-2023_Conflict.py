...
def GetDistanceMatrix_Parrallel(Parcours_dict, Aggreg_parameters,nthreads):
    ...
    for i in range(len(List_Cn)-2):
        result[i]=Distance_matrix_compute(List_Cn[i],List_Cn[i+1],data,Nb_dim,Nb_NIP)
...
def Distance_matrix_compute(Start_col,End_col,data,Nb_dim,Nb_lines):
    ...
    Partial_dist_matrix = np.zeros((End_col-Start_col, Nb_lines))
    Partial_dist_matrix_T = np.zeros((Nb_lines,End_col-Start_col))

    for i in range(0,End_col-Start_col):
        for j in range(i,End_col):   #Compute only half of the matrix as the distance is symetric !
            
            line_i=(Start_col+i)*Nb_dim
            line_j=(Start_col+j)*Nb_dim
            ...
            Partial_dist_matrix[i, j] = dtw.dtw(query, template, distance_only=True).distance
            Partial_dist_matrix_T[j, i] = Partial_dist_matrix[i, j]
